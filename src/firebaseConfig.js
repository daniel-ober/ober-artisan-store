...
// ✅ DEV DIAGNOSTICS (safe: logs only lengths / endings)
if (typeof window !== "undefined" && process.env.NODE_ENV !== "production") {
  ...
  console.log(
    ...
    k ? String(k).length : 0,
    "| endsWith:",
    k ? String(k).slice(-6) : "n/a"
  );
  console.log("[firebase] authDomain:", firebaseConfig.authDomain);
}

// 🚨 Fail fast with a clear message instead of white screen mystery
if (!firebaseConfig.apiKey) {
  throw new Error(
    "[firebase] Missing REACT_APP_FIREBASE_API_KEY at build time. " +
      "Check .env.local and restart `npm start`."
  );
}
...
// In dev, allow App Check debug token (prevents App Check from blocking)
if (typeof window !== "undefined" && process.env.NODE_ENV !== "production") {
  // Use globalThis to avoid self/window weirdness
...
// Note: setPersistence returns a promise; we don't block app boot on it.
