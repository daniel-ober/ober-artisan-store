...
const CheckoutSummary = () => {
  ...
  const userId = queryParams.get('userId'); // For authenticated users
  ...
  useEffect(() => {
    const fetchOrderDetails = async () => {
      ...
      try {
        ...
        if (!existingOrder) {
          // Fetch the order from the backend and save it to Firestore
  ...
  const fetchOrderFromBackend = async (stripeSessionId) => {
    try {
      ...
      if (!response.ok) throw new Error('Failed to fetch order from backend.');
  ...
  const saveOrderToFirestore = async (orderData) => {
    try {
      ...
      await addDoc(ordersRef, {
        ...orderData,
        createdAt: new Date(),
        userId: userId || null, // Attach userId for authenticated users
      });
    } catch (error) {
