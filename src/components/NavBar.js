...
import { FaCartPlus, FaCog, FaSignOutAlt } from 'react-icons/fa';
...
const VOICE_PLAYGROUND_ROUTE = '/legacyprint/voice-playground';
...
const NavBar = () => {
  ...
  useEffect(() => {
    const sync = () => setImpersonateUid(sessionStorage.getItem('impersonateUid') || '');
  ...
  const canShowSoundLegendUI = (!isAdmin && hasSLClaim) || (isAdmin && isImpersonating);
  ...
  useEffect(() => {
    const fetchNavbarLinks = async () => {
      try {
        ...
        const homeLink = links.find((l) => (l.name || '').toLowerCase() === 'home');

        const ourCraftLink = links.find((l) => (l.name || '').toLowerCase() === 'our-craft');
        ...
        const merchLink = links.find((l) => (l.name || '').toLowerCase() === 'merch');
        ...
        const transformed = links
          ...
          .filter((l) => {
            ...
            return (
              lower !== 'artisan-shop' && lower !== 'founders-batch' && lower !== 'founders-toast'
            );
          })
  ...
  const renderCartButton = () => (
    ...
    >
      ...
      <span className="cart-badge" style={{ display: cartItemCount > 0 ? 'inline-block' : 'none' }}>
        {cartItemCount}
      </span>
    </button>
  );
  ...
  const filteredLinks = navbarLinks.filter((link) => {
    ...
    const isFoundersToast = name.includes('founders-toast') || label.includes('founder’s toast');
  ...
  const renderVoicePlaygroundLink = () => {
    if (!user || !isAdmin) return null;

    return (
      <Link
        to={VOICE_PLAYGROUND_ROUTE}
        className="nav-link"
        onClick={() => handleNavLinkClick(VOICE_PLAYGROUND_ROUTE)}
        title="LegacyPrint Voice Playground"
      >
        Voice Playground
      </Link>
    );
