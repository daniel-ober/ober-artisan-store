...
const LegacyPrintInteractivePolygon = ({
  ...
  const [internalPlayValues, setInternalPlayValues] = useState(DEFAULT_PLAY_VALUES);
  ...
  const currentPlayValues = normalizePlayValues(playValues || internalPlayValues);
  ...
  const hoveredNode = outerNodePoints.find((node) => node.key === hoveredNodeKey);
  ...
  const getPointerSliceInfo = (point) => {
    ...
    const index = clamp(Math.floor(normalized / step), 0, NODE_ORDER.length - 1);
  ...
  const updateVoiceSculptorFromPointer = (event, interactionOverride = null) => {
