...
const AddUserModal = ({ onClose, onUserAdded }) => {
  ...
  const [generatedPassword, setGeneratedPassword] = useState(generateThemedPassword());
  ...
  async function ensureAdminAndFreshToken() {
    ...
    await user.reload();
    await user.getIdToken(true); // refresh so callables get a fresh token w/ claims
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      console.log('[adminCreateUser] calling', { email, passwordMasked: '***' });
