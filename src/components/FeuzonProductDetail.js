...
const FeuzonProductDetail = () => {
  ...
  const [isLoadingProductAvailability, setIsLoadingProductAvailability] = useState(true);
  ...
  const handleChangeSelections = () => {
    ...
    const existingItemIndex = cart.findIndex(item => item.productId === "feuzon");
    if (existingItemIndex !== -1) {
        console.log("♻️ Removing Feuzon item temporarily to allow selection update.");
...
const handleAddToCart = async (addAsSeparateItem = false) => {
  ...
  let cartItem = {
    ...
    quantity: 1, // Always 1 per variation
  };
...
useEffect(() => {
  ...
  console.log("🗂️ Available keys in feuzonSummaries:", Object.keys(feuzonSummaries)); // ✅ Debugging log
  ...
  const selectedOption = feuzonSummaries.pricingOptions.find(option =>
  ...
  if (!selectedOption) {
      console.error("❌ No matching pricing option found for:", { size: normalizedSize, depth: normalizedDepth, reRing: hasReRing, lugs });
      setTotalPrice(0);
  ...
  let staveThickness = staveParts.length > 1 ? staveParts[1]?.trim() : "UNKNOWN_THICKNESS";

  if (staveOption.includes("10 - 10mm") && !selectedOption.reRing) {
      console.log("➕ Adding Re-Rings Cost: $150 for 10 Lugs 10mm (Only if not included)");
      setTotalPrice(prevPrice => prevPrice + reRingCost);
      staveThickness = "10mm + Re-Rings + $150 (Re-Rings Required)";
  }

  console.log(`💰 Updated Price: $${selectedOption.price}`);

  // ✅ Ensure correct lookup key for artisan notes
  const formattedBasePrice = selectedOption.price.toString().trim();
  const formattedDepth = depth.toString().trim() + '"';
  const formattedLugs = lugs.toString().trim() + " Lugs";
  const formattedOuterShell = outerShell.trim();
  const formattedInnerStave = innerStave.trim();

  const generatedKey = `${size}" - Base Price: $${formattedBasePrice}-${formattedDepth}-${formattedLugs}-${staveCount} - ${staveThickness}-${formattedOuterShell}-${formattedInnerStave}`
  ...
  if (Object.prototype.hasOwnProperty.call(feuzonSummaries, generatedKey)) {
      ...
      console.warn(`⚠️ No summary found for ${generatedKey}. Using fallback summary.`);
...
}, [size, depth, lugs, staveOption, outerShell, innerStave]); // ✅ Runs when these change
...
useEffect(() => {
  ...
  console.log("🔍 Checking if product is in cart:", isInCart, "for ID:", generatedId);
  console.log("🔍 Current Cart Contents:", cart.map(item => item.id));
...
useEffect(() => {
  console.log(`🔄 Resetting Depth, Lugs, and Stave Option due to Snare Size or Lug Change: ${size}, ${lugs}`);
  ...
  if (staveMapping[size]?.[lugs]) {
      ...
      if (!validStaveOptions.includes(staveOption)) {
          const defaultStaveOption = validStaveOptions.length > 0 ? validStaveOptions[0] : "";
  ...
  const fetchProductAvailability = async () => {
    ...
    try {
      ...
      if (productSnap.exists()) {
        ...
        console.log("📦 Firestore Feuzon Data:", productData);
        setProductQuantity(productData.currentQuantity || 0);
      } else {
...
const handleAddSeparateItem = async () => {
  console.log("➕ Adding a separate Feuzon drum to the cart...");

  // ✅ Check if we have enough stock to add an extra Feuzon
  if (currentQuantity > cart.filter(item => item.productId === "feuzon").length) {
    await handleAddToCart(true);
    setShowAddSeparateModal(false);
  } else {
    alert("Not enough stock available to add another Feuzon.");
    console.error("❌ Cannot add additional Feuzon, stock limit reached.");
  }
};
...
<div className="button-group">
        {productQuantity === 0 ? (
          <button className="notify-button">Sold Out - Notify Me When Available</button>
        ) : productInCart ? (
          <>
            <button className="go-to-checkout-button" onClick={() => window.location.href = "/cart"}>
              Go to Checkout
            </button>

            <button className="change-selections-button" onClick={() => setShowModifyModal(true)}>
              Change Selections
            </button>

            <button className="remove-from-cart-button" onClick={async () => {
...
{selectionChanged && (
  ...
  <button onClick={handleAddSeparateItem}>Add Separate Feuzon</button>
)}
      </div>
    </div>
  </div>
...
{showModifyModal && (
  <div className="modal-overlay">
    <div className="modal-content">
      ...
      <button onClick={handleModifySelection}>Replace Current Selection</button>
    </div>
  </div>
)}
...
{showAddSeparateModal && (
  <div className="modal-overlay">
    <div className="modal-content">
      ...
      <p>
        You already have a Feuzon snare in your cart. Would you like to add this as a separate item or replace your current selection?
      </p>
      <button onClick={() => setShowAddSeparateModal(false)}>Cancel</button>
      <button onClick={handleAddSeparateItem}>Add as Separate Item</button>
      <button onClick={handleModifySelection}>Replace Current Selection</button>
    </div>
  </div>
)}
