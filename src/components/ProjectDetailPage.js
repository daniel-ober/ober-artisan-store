...
const ProjectDetailPage = () => {
  ...
  // ---------- static config ----------
  const stepWeights = {
    woodPreparation: 0.05, shellConstruction: 0.2, fineTuning: 0.1, shellExteriorFinish: 0.2,
    bearingEdges: 0.1, snareBedCutting: 0.1, hardwareDrilling: 0.1, hardwareAssembly: 0.05,
    tuningAndDetailing: 0.05, qualityCheck: 0.05,
  };

  const allFileSections = [
    'build_proposal','wood_selection','early_mockups_(pre-production)',
    'stave_construction_(pre-milling)','stave_construction_(post-milling)',
    'final_mockups_(mid-production)','media_files_(audio/video)','other',
  ];

  const STEP_ORDER = [
    'woodPreparation','shellConstruction','fineTuning','shellExteriorFinish','bearingEdges',
    'snareBedCutting','hardwareDrilling','hardwareAssembly','tuningAndDetailing','qualityCheck',
  ];

  const TOOL_LIBRARY = {
    'Moisture meter': { img: '/tools/moisture-meter.jpg', desc: 'Checks wood moisture to prevent movement.' },
    'Jointer & planer': { img: '/tools/jointer-planer.jpg', desc: 'True faces & thickness boards.' },
    Calipers: { img: '/tools/calipers.jpg', desc: 'Measure diameters & thickness precisely.' },
    Clamps: { img: '/tools/clamps.jpg', desc: 'Even pressure during glue-ups.' },
    'Table saw + bevel sled': { img: '/tools/table-saw.jpg', desc: 'Accurate stave bevels/angles.' },
    'Clamping forms': { img: '/tools/forms.jpg', desc: 'Keeps shells round while curing.' },
    'Dial calipers': { img: '/tools/calipers.jpg', desc: 'Verify roundness & diameters.' },
    'Lathe/drum sander': { img: '/tools/lathe.jpg', desc: 'True faces & profile thickness.' },
    'Dial indicator': { img: '/tools/dial-indicator.jpg', desc: 'Read tiny deviations for trueness.' },
    'Sanding blocks': { img: '/tools/sanding-blocks.jpg', desc: 'Controlled surface refinement.' },
    'HVLP sprayer': { img: '/tools/hvlp.jpg', desc: 'Thin, even finish coats.' },
    'Polishing system': { img: '/tools/polish.jpg', desc: 'Level & bring sheen to spec.' },
    'Viscosity cups': { img: '/tools/cup.jpg', desc: 'Consistent finish flow rates.' },
    'Router table & jigs': { img: '/tools/router.jpg', desc: 'Cut bearing edges/snare beds.' },
    Files: { img: '/tools/files.jpg', desc: 'Refine edges & holes.' },
    'Burnish wheels': { img: '/tools/burnish.jpg', desc: 'Silky head glide on edges.' },
    'Router sled': { img: '/tools/router-sled.jpg', desc: 'Even bed depth & transitions.' },
    'Feeler gauges': { img: '/tools/feeler.jpg', desc: 'Verify bed clearance.' },
    'Blocks & abrasives': { img: '/tools/blocks.jpg', desc: 'Blend profiles cleanly.' },
    'Drill press + jigs': { img: '/tools/drill-press.jpg', desc: 'Aligned hardware holes.' },
    'Step bits': { img: '/tools/step-bit.jpg', desc: 'Clean, tear-out-free drilling.' },
    'Layout templates': { img: '/tools/templates.jpg', desc: 'Repeatable placements.' },
    'Torque drivers': { img: '/tools/driver.jpg', desc: 'Even, repeatable torque.' },
    'Soft jaws': { img: '/tools/soft-jaws.jpg', desc: 'Protect finish/hardware.' },
    'Thread treatments': { img: '/tools/thread.jpg', desc: 'Quiet, stable hardware.' },
    'Tension gauge': { img: '/tools/tension.jpg', desc: 'Check lug balance.' },
    'Reference tuner': { img: '/tools/tuner.jpg', desc: 'Set target pitches.' },
    Straightedges: { img: '/tools/straightedge.jpg', desc: 'Confirm flatness/edges.' },
    'Reference mic': { img: '/tools/mic.jpg', desc: 'QC audio checks.' },
    Monitors: { img: '/tools/monitors.jpg', desc: 'Listen for rattles/buzzes.' },
    'Inspection lights': { img: '/tools/light.jpg', desc: 'Reveal finish defects.' },
  };
  ...
  const STEP_EDU = {
    woodPreparation: {
      ...
      qc: ['Moisture 6–9%','Faces flat & parallel','Grain orientation marked'],
  ...
  const currentIndexFromPhase = (currentPhase) => {
    ...
    const foundKey = STEP_ORDER.find((k) => lower.includes(STEP_EDU[k].title.toLowerCase()));
  ...
  useEffect(() => {
    const fetchProject = async () => {
      ...
      try {
        ...
        if (!snap.exists()) { navigate('/not-found'); return; }
        ...
        if (isAdmin || data.ownerUid === user.uid) {
          ...
          Object.entries(raw).forEach(([key, files]) => {
            ...
            files.forEach((file) => {
              const f = typeof file === 'string' ? { url: file, hidden: false, category: key } : file;
  ...
  // ---------- derive values (SAFE DEFAULTS so hooks always run) ----------
  ...
  // ---- calendar ETA (scaled against the standard 8–10 weeks, not exploding with WIP) ----
  const BASE_WEEKS = { min: 8, max: 10, mid: 9 };

  // remaining fraction of the build (by working hours)
  ...
  // calendar weeks left based on the shop's typical 8–10 week turnaround
  ...
  // ---------- helpers ----------
  const renderHourRange = (t) => (!t ? '—' : (t.min === t.max ? `${t.min} hrs` : `${t.min}–${t.max} hrs`));

  const ToolItem = ({ name }) => {
    const meta = TOOL_LIBRARY[name] || {};
    return (
      <li className="tool-item">
        <span className="tool-link" aria-describedby={`tip-${name}`}>{name}</span>
        <div className="tool-pop" id={`tip-${name}`} role="tooltip">
          {meta.img && <img src={meta.img} alt={name} onError={(e)=>{e.currentTarget.style.display='none';}} />}
          <div className="tool-name">{name}</div>
          <div className="tool-desc">{meta.desc || 'Shop tool used in this operation.'}</div>
        </div>
      </li>
    );
  };
  ...
  const renderTinyThumb = (file, i) => {
    ...
    if (!url) return null;
    const name = decodeURIComponent(url.split('?')[0].split('/').pop()?.split('%2F').pop() || 'file');
    const ext = name.includes('.') ? name.split('.').pop().toLowerCase() : '';
    const isImage = ['jpg','jpeg','png','gif','webp'].includes(ext);
    const isVideo = ['mp4','webm','mov'].includes(ext);
    const isPDF = ext === 'pdf';
    const isAudio = ['mp3','wav','ogg'].includes(ext);
    const open = () => { setIsPreviewLoaded(false); setModalPreview({ url, ext }); };

    return (
      <div key={`${name}-${i}`} className="mini-thumb" onClick={open} role="button" tabIndex={0}
           onKeyDown={(e)=>{ if (e.key==='Enter' || e.key===' ') open(); }} title={name}>
        {isImage && <img src={url} alt={name} />}
        {isVideo && <video muted loop playsInline><source src={url} /></video>}
        {isPDF && <div className="mini-pdf">PDF</div>}
        {isAudio && <div className="mini-audio">🔊</div>}
        {!isImage && !isVideo && !isPDF && !isAudio && <div className="mini-file">FILE</div>}
      </div>
    );
  };
      ...
      await new Promise((resolve, reject) => {
        ...
        task.on('state_changed', () => {},
          ...
          async () => {
            ...
            try {
              await updateDoc(doc(db, 'projects', project.id), { [`attachments.${cat}`]: nextList });
  ...
  // ---------- SAFE conditional UI (after all hooks) ----------
  ...
  return (
    <div className="project-page">
      ...
      <section className="project-section">
        ...
        {/* details */}
        {!selectedStep ? (
          <p className="muted" style={{ marginTop: '12px' }}>Select a step above to see what happens there.</p>
        ) : (() => {
          ...
          return (
            <article className={['edu-step', unlocked ? 'unlocked' : 'locked', active ? 'active' : '', complete ? 'complete' : ''].join(' ')} style={{ marginTop: '14px' }}>
              <header className="edu-step-head">
                <div className="edu-header-card">
                  ...
                  <div className="edu-titles">
                    <h4>{meta.title}</h4>
                    <span className="edu-state">{complete ? 'Completed' : active ? 'In Progress' : 'Locked'}</span>
                    <div className="eta-row">
                      <div className="eta-chip"><span className="sub">Standard Turnaround</span><span className="big">8–10 weeks</span></div>
                      <div className="eta-chip"><span className="sub">Projected Completion (throughput)</span><span className="big">{etaDateRange}</span></div>
                    </div>
                  </div>
      ...
      <section className="project-section">
        ...
        <p><strong>Shipping Address:</strong> {customer?.address ? [customer.address.street, customer.address.city, customer.address.state, customer.address.zip].filter(Boolean).join(', ') : 'N/A'}</p>
        {!isAdmin && <button className="edit-button" onClick={() => window.open('mailto:soundlegend@oberartisandrums.com?subject=Request to update customer info','_blank')}>Request Changes</button>}
      </section>
      ...
      <section className="project-section">
        ...
        {project?.reinforcementRings !== 'None' && <p><strong>Re-Rings Wood Species:</strong> {project?.reringsSpecies && project.reringsSpecies !== 'None' ? project.reringsSpecies : 'N/A'}</p>}
          ...
          <section className="project-section" key={sectionKey}>
            ...
            <div className="file-preview-grid">
              {files.map((file, i) => {
                ...
                const isPDF = ext === 'pdf';
                return (
                  <div key={i} className="file-preview-item" onClick={() => { setIsPreviewLoaded(false); setModalPreview({ url, ext }); }} style={{ cursor: 'pointer' }}>
                    {isImage ? (
                      <img src={url} alt={fileName} className="file-preview-image" style={{ height:'160px', objectFit:'cover', borderRadius:'8px', border:'1px solid #444' }}/>
                    ) : (
        ...
        <div className="file-preview-modal" onClick={() => setModalPreview(null)}>
          <div className="file-preview-modal-content" onClick={(e) => e.stopPropagation()}>
            ...
            {modalPreview.ext === 'pdf' ? (
              <iframe src={modalPreview.url} title="PDF Preview" className="file-preview-pdf"
                      style={{ visibility: isPreviewLoaded ? 'visible' : 'hidden', opacity: isPreviewLoaded ? 1 : 0, transition: 'opacity .4s ease' }}
                      onLoad={() => setIsPreviewLoaded(true)} />
            ) : ['mp4','webm','mov'].includes(modalPreview.ext) ? (
              <video controls autoPlay loop className="file-preview-video"
                     style={{ visibility: isPreviewLoaded ? 'visible' : 'hidden', opacity: isPreviewLoaded ? 1 : 0, transition: 'opacity .4s ease' }}
              ...
              <img src={modalPreview.url} alt="Preview" className="file-preview-image"
                   style={{ visibility: isPreviewLoaded ? 'visible' : 'hidden', opacity: isPreviewLoaded ? 1 : 0, transition: 'opacity .4s ease' }}
                   onLoad={() => setIsPreviewLoaded(true)} />
            )}
          </div>
