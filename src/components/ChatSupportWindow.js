...
function ChatSupportWindow({ currentTab, messages, sendMessage, toggleMaximize, isMaximized, toggleChat }) {
    ...
    useEffect(() => {
        ...
        if (currentTab && preloadedQuestions[currentTab]) {
            setPreloadedQuestionsList(preloadedQuestions[currentTab]);
            console.log("Preloaded Questions List:", preloadedQuestions[currentTab]);
        } else {
        ...
        <div className="chat-support-window">
            ...
            <div className="preloaded-questions">
                {preloadedQuestionsList.length > 0 ? (
                    preloadedQuestionsList.map((question, index) => (
                        <button
                            key={index}
