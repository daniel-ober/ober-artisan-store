...
const scoreByMaterial = (material, construction, depth = 5.5) => {
  ...
  if (m.includes('brass')) {
  ...
  } else if (m.includes('aluminum') || m.includes('acrolite') || m.includes('ludalloy')) {
    ...
    s = { attack: 7.1, brightness: 6.4, projection: 7.4, sustain: 6.9, warmth: 7.5, sensitivity: 7.2, control: 6.5 };
    ...
    s = { attack: 6.7, brightness: 6.1, projection: 6.6, sustain: 6.5, warmth: 7.5, sensitivity: 6.8, control: 6.6 };

  } else if (m.includes('maple')) {

    s = { attack: 7.0, brightness: 6.7, projection: 7.0, sustain: 6.4, warmth: 7.0, sensitivity: 7.0, control: 6.7 };

  } else if (m.includes('mahogany')) {

    s = { attack: 6.2, brightness: 5.7, projection: 6.4, sustain: 6.7, warmth: 7.8, sensitivity: 6.6, control: 6.5 };
...
for (const doc of snap.docs) {
  ...
  const patch = {
    ...
    scoringBasis:
      ...
      'Shell-first scoring generated from nested Ludwig shell/reference fields: dimensions, shell construction, shell materials, bearing edge family, reinforcement rings, snare bed availability, and hoop type. Brand prestige, rarity, resale value, collectibility, and artist association are excluded.',

    engineReadinessNotes:

      d.engineReadinessNotes ||

      'Ludwig record normalized from existing nested shell, stock hardware, and stock snare system data. Unknown values are preserved where source confirmation is not available.',

    dataCleanupUpdatedAt: admin.firestore.FieldValue.serverTimestamp(),

    dataCleanupUpdatedBy: 'normalizeLudwigFromNestedData'
...
console.log(`Done. Updated ${updated} Ludwig records.`);
