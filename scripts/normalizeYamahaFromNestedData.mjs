...
const scoreByMaterial = (material, construction, depth = 5.5) => {
  ...
  if (m.includes('maple')) s = { attack: 7.0, brightness: 6.7, projection: 7.0, sustain: 6.4, warmth: 7.0, sensitivity: 7.0, control: 6.7 };

  if (m.includes('birch')) s = { attack: 7.4, brightness: 7.2, projection: 7.5, sustain: 6.1, warmth: 6.1, sensitivity: 7.0, control: 6.8 };

  if (m.includes('oak')) s = { attack: 7.5, brightness: 7.0, projection: 8.0, sustain: 6.6, warmth: 6.6, sensitivity: 6.8, control: 6.5 };

  if (m.includes('beech')) s = { attack: 7.1, brightness: 6.6, projection: 7.2, sustain: 6.5, warmth: 6.9, sensitivity: 7.0, control: 6.7 };

  if (m.includes('steel')) s = { attack: 7.9, brightness: 8.0, projection: 8.0, sustain: 6.5, warmth: 5.6, sensitivity: 7.0, control: 6.4 };

  if (m.includes('brass')) s = { attack: 7.7, brightness: 7.3, projection: 8.1, sustain: 7.0, warmth: 6.9, sensitivity: 7.1, control: 6.3 };

  if (m.includes('aluminum')) s = { attack: 7.2, brightness: 6.7, projection: 7.0, sustain: 5.8, warmth: 6.2, sensitivity: 7.6, control: 7.2 };

  if (m.includes('copper')) s = { attack: 7.1, brightness: 6.4, projection: 7.4, sustain: 6.9, warmth: 7.5, sensitivity: 7.2, control: 6.5 };
...
const snap = await db.collection('snareReferenceDrums').where('companyName', '==', 'Yamaha').get();
...
for (const doc of snap.docs) {
  ...
  const patch = {
    ...
    scoringBasis: d.scoringBasis || 'Shell-first scoring generated from nested Yamaha shell/reference fields. Brand prestige, rarity, resale value, collectibility, and artist association are excluded.',

    engineReadinessNotes: d.engineReadinessNotes || 'Yamaha record normalized from existing nested shell, stock hardware, and stock snare system data. Unknown values are preserved where source confirmation is not available.',

    needsResearch: d.needsResearch === true ? true : false,

    dataCleanupUpdatedAt: admin.firestore.FieldValue.serverTimestamp(),

    dataCleanupUpdatedBy: 'normalizeYamahaFromNestedData'
...
console.log(`Done. Updated ${updated} Yamaha records.`);
