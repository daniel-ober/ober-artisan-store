...
for (const doc of snap.docs) {
  ...
  const patch = {
    ...
    scoringBasis:
      ...
      'Shell-first scoring generated from nested Mapex shell/reference fields: dimensions, shell construction, shell materials, bearing edge family where available, reinforcement rings, snare bed availability, and hoop type. Brand prestige, rarity, resale value, collectibility, and artist association are excluded.',

    engineReadinessNotes:

      d.engineReadinessNotes ||

      'Mapex record normalized from existing nested shell, stock hardware, and stock snare system data. Unknown values are preserved where source confirmation is not available.',

    needsResearch: d.needsResearch === true ? true : false,

    dataCleanupUpdatedAt: admin.firestore.FieldValue.serverTimestamp(),

    dataCleanupUpdatedBy: 'normalizeMapexFromNestedData'
...
console.log(`Done. Updated ${updated} Mapex records.`);
