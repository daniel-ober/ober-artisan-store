...
const scoreByMaterial = (material, construction, depth = 5.5) => {
  ...
  if (m.includes('acrylic')) s = { attack: 7.5, brightness: 7.3, projection: 7.8, sustain: 7.1, warmth: 5.8, sensitivity: 6.8, control: 6.2 };

  else if (m.includes('maple') && m.includes('mahogany')) s = { attack: 6.7, brightness: 6.2, projection: 6.9, sustain: 6.8, warmth: 7.7, sensitivity: 6.9, control: 6.5 };
  ...
  else if (m.includes('maple') && m.includes('poplar')) s = { attack: 6.7, brightness: 6.1, projection: 6.6, sustain: 6.5, warmth: 7.4, sensitivity: 6.8, control: 6.6 };

  else if (m.includes('mahogany') && m.includes('poplar')) s = { attack: 6.3, brightness: 5.8, projection: 6.5, sustain: 6.6, warmth: 7.6, sensitivity: 6.6, control: 6.5 };
  ...
  else if (m.includes('beech')) s = { attack: 7.1, brightness: 6.6, projection: 7.2, sustain: 6.5, warmth: 6.9, sensitivity: 7.0, control: 6.7 };

  else if (m.includes('walnut')) s = { attack: 6.8, brightness: 6.3, projection: 7.0, sustain: 6.8, warmth: 7.5, sensitivity: 6.8, control: 6.6 };

  else if (m.includes('mahogany')) s = { attack: 6.2, brightness: 5.7, projection: 6.4, sustain: 6.7, warmth: 7.8, sensitivity: 6.6, control: 6.5 };

  else if (m.includes('steel')) s = { attack: 7.9, brightness: 8.0, projection: 8.0, sustain: 6.5, warmth: 5.6, sensitivity: 7.0, control: 6.4 };

  else if (m.includes('brass') || m.includes('bell brass')) s = { attack: 7.7, brightness: 7.3, projection: 8.1, sustain: 7.0, warmth: 6.9, sensitivity: 7.1, control: 6.3 };
  ...
  else if (m.includes('copper')) s = { attack: 7.1, brightness: 6.4, projection: 7.4, sustain: 6.9, warmth: 7.5, sensitivity: 7.2, control: 6.5 };
  ...
  if (c.includes('solid') || c.includes('stave')) {
...
for (const company of COMPANIES) {
  ...
  const snap = await db.collection('snareReferenceDrums').where('companyName', '==', company).get();
  ...
  for (const doc of snap.docs) {
    ...
    const patch = {
      ...
      scoringBasis: d.scoringBasis || `Shell-first scoring generated from nested ${company} shell/reference fields. Brand prestige, rarity, resale value, collectibility, country of origin, price, and artist association are excluded.`,

      engineReadinessNotes: d.engineReadinessNotes || `${company} record normalized from existing nested shell, stock hardware, and stock snare system data. Unknown values are preserved where source confirmation is not available.`,

      needsResearch: d.needsResearch === true ? true : false,

      dataCleanupUpdatedAt: admin.firestore.FieldValue.serverTimestamp(),

      dataCleanupUpdatedBy: 'normalizeAllRemainingSnareCompanies'
  ...
  console.log(`Updated ${companyUpdated}: ${company}`);

}

console.log(`Done. Updated ${totalUpdated} total records.`);
