...
const scoreByMaterial = (material, construction, depth = 5.5) => {
  ...
  if (m.includes('acrylic')) {
    ...
    s = { attack: 7.5, brightness: 7.3, projection: 7.8, sustain: 7.1, warmth: 5.8, sensitivity: 6.8, control: 6.2 };

  } else if (m.includes('maple') && m.includes('mahogany')) {

    s = { attack: 6.7, brightness: 6.2, projection: 6.9, sustain: 6.8, warmth: 7.7, sensitivity: 6.9, control: 6.5 };

  } else if (m.includes('maple') && m.includes('walnut')) {

    s = { attack: 6.9, brightness: 6.4, projection: 7.1, sustain: 6.9, warmth: 7.4, sensitivity: 6.9, control: 6.6 };

  } else if (m.includes('maple') && m.includes('poplar')) {

    s = { attack: 6.7, brightness: 6.1, projection: 6.6, sustain: 6.5, warmth: 7.4, sensitivity: 6.8, control: 6.6 };

  } else if (m.includes('mahogany') && m.includes('poplar')) {

    s = { attack: 6.3, brightness: 5.8, projection: 6.5, sustain: 6.6, warmth: 7.6, sensitivity: 6.6, control: 6.5 };

  } else if (m.includes('maple')) {

    s = { attack: 7.0, brightness: 6.7, projection: 7.0, sustain: 6.4, warmth: 7.0, sensitivity: 7.0, control: 6.7 };

  } else if (m.includes('birch')) {

    s = { attack: 7.4, brightness: 7.2, projection: 7.5, sustain: 6.1, warmth: 6.1, sensitivity: 7.0, control: 6.8 };

  } else if (m.includes('steel')) {

    s = { attack: 7.9, brightness: 8.0, projection: 8.0, sustain: 6.5, warmth: 5.6, sensitivity: 7.0, control: 6.4 };

  } else if (m.includes('brass')) {

    s = { attack: 7.7, brightness: 7.3, projection: 8.1, sustain: 7.0, warmth: 6.9, sensitivity: 7.1, control: 6.3 };

  } else if (m.includes('aluminum')) {

    s = { attack: 7.2, brightness: 6.7, projection: 7.0, sustain: 5.8, warmth: 6.2, sensitivity: 7.6, control: 7.2 };

  } else if (m.includes('bronze')) {

    s = { attack: 7.5, brightness: 7.0, projection: 8.0, sustain: 7.2, warmth: 7.2, sensitivity: 7.2, control: 6.4 };
...
for (const doc of snap.docs) {
  ...
  const scores = scoreByMaterial(combinedMaterial, shellConstruction, depth || 5.5);
  ...
  const patch = {
    ...
    reinforcementRings:
      ...
      typeof d.reinforcementRings === 'boolean'
        ...
        : boolFromUnknown(construction.reinforcementRings, false),
    ...
    scoringBasis:
      ...
      'Shell-first scoring generated from nested Pearl shell/reference fields: dimensions, shell construction, shell materials, bearing edge family where available, reinforcement rings, snare bed availability, and hoop type. Brand prestige, rarity, resale value, collectibility, and artist association are excluded.',

    engineReadinessNotes:

      d.engineReadinessNotes ||

      'Pearl record normalized from existing nested shell, stock hardware, and stock snare system data. Unknown values are preserved where source confirmation is not available.',

    needsResearch: d.needsResearch === true ? true : false,

    dataCleanupUpdatedAt: admin.firestore.FieldValue.serverTimestamp(),

    dataCleanupUpdatedBy: 'normalizePearlFromNestedData'
...
console.log(`Done. Updated ${updated} Pearl records.`);
