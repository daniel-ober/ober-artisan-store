...
const snap = await db.collection('snareReferenceDrums').where('companyName', '==', 'SONOR').get();
...
for (const doc of snap.docs) {
  ...
  const patch = {
    ...
    scoringBasis: d.scoringBasis || 'Shell-first scoring generated from nested SONOR shell/reference fields. Brand prestige, rarity, resale value, collectibility, and artist association are excluded.',

    engineReadinessNotes: d.engineReadinessNotes || 'SONOR record normalized from existing nested shell, stock hardware, and stock snare system data. Unknown values are preserved where source confirmation is not available.',

    needsResearch: d.needsResearch === true ? true : false,

    dataCleanupUpdatedAt: admin.firestore.FieldValue.serverTimestamp(),

    dataCleanupUpdatedBy: 'normalizeSONORFromNestedData'
...
console.log(`Done. Updated ${updated} SONOR records.`);
